...
export const NewsListItem: React.FC<NewsListItemProps> = ({
  ...
  const isDifferentLanguage = article.language && !article.language.startsWith(locale.split('-')[0])

  return (
    <article
      onClick={() => onArticleClick(article)}
      className="group cursor-pointer flex items-start gap-5 py-6 border-b border-border/60"
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 text-[11px] text-text-muted">
          <span className="font-semibold truncate">{article.sourceName}</span>
          <span>•</span>
          <time className="tabular-nums shrink-0">{formatRelativeTime(article.publishedAt, t)}</time>
          {isDifferentLanguage && <span className="shrink-0">({article.language})</span>}
        </div>

        <h3 className="mt-1.5 text-base font-bold leading-snug tracking-tight text-text group-hover:text-accent transition-colors line-clamp-3">
          {article.title}
        </h3>
        ...
        {/* Altura reservada: as ações aparecem no hover sem empurrar o layout */}
        <div className="mt-2 h-6 flex items-center">
          <div className="hidden group-hover:flex items-center gap-1 text-text-muted">
            ...
            <button
              ...
              title={t('app.like')}
              className={`p-1 transition-colors ${feedback === 'like' ? 'text-accent' : 'hover:text-text'}`}
            >
            ...
            <button
              ...
              title={t('app.dislike')}
              className={`p-1 transition-colors ${feedback === 'dislike' ? 'text-error' : 'hover:text-text'}`}
            >
            ...
            <button
              ...
              className={`p-1 transition-colors ${isSaved ? 'text-accent' : 'hover:text-text'}`}
            >
      ...
      {article.image && !imageError && (
        <div className="w-40 h-28 shrink-0 overflow-hidden rounded-lg bg-sidebar">
          <img
            src={article.image}
            alt={article.title}
